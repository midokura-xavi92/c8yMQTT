...
class DeviceProxy:
    ...
    def _tcp_port_connect(self, host, port):
        ...
        logging.info(f"Connected to TCP Host {host} with Port {port} successfully.")
    ...
    def _websocket_connect(self, connection_key):
        ...
        if self.token:
            ...
            raise WebSocketFailureException(
                "OAuth Token or tenant, user and password must be provided!"
            )
        ...
        wst = threading.Thread(
            target=self._web_socket.run_forever,
            kwargs={"ping_interval": 10, "ping_timeout": 7},
        )
        wst.daemon = True
