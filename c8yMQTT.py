...
class C8yMQTT(object):
    ...
    def __init__(
        ...
        self.logHandler = RotatingFileHandler(
            "c8yAgent.log", maxBytes=1 * 1024 * 1024, backupCount=5
        )
        ...
        if not self.cert_auth:
            ...
            if not os.path.exists(self.configFile):
                ...
                self.logger.error(
                    "Config file does not exist, please call registerDevice() of edit Config: "
                    + self.configFile
                )
                return
            ...
            if (
                ...
                or self.tenant == ""
                or self.clientId == ""
            ) and not self.cert_auth:
                self.logger.error(
                    "Coould not  initialize Agent. Missing Values in c8y.properties"
                )
                self.initialized = False
            else:
            ...
            self.logger.info(
                "Using certificate authentication. Successfully initialized."
            )
            self.initialized = True
    ...
    def subscribe_topics(self, topics, qos=0):
        ...
        for t in topics:
            try:
                self.logger.debug(
                    "Subscribing to topic " + str(t) + " qos: " + str(qos)
                )
                ...
                if r[0] == 0:
                    ...
                    self.logger.error(
                        "error on subscribing: " + t + " return code:" + str(r)
                    )
    ...
    def connect(self, on_message, topics):
        ...
        if self.initialized == False:
            self.logger.error(
                "Not initialized, please call bootstrap() of edit c8y.properties file. Alternatively you can use cert auth."
            )
            return
        ...
        if self.cert_auth:
            ...
            self.logger.debug(
                "Using Basic Authentication withe Creds: "
                + self.tenant
                + "/"
                + self.user
                + " pwd: "
        ...
        if not self.connected == 0:
            self.logger.debug(
                "Connect not successfull return to client. Code:" + str(self.connected)
            )
    ...
    def initDevice(
        ...
        self.client.publish(
            "s/us", "100," + self.deviceName + "," + self.deviceType, 2
        ).wait_for_publish()
        self.client.publish(
            "s/us",
            "110,"
    ...
    def __on_message_createdevice(self, client, userdata, message):
        message = message.payload.decode("utf-8")
        self.logger.info(
            "__on_message_createdevice Received Registration Message: " + message
        )
